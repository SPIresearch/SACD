...
def get_opt():
    ...
    if opt.root_path is not None:
        ...
        if opt.eeg_path is not None:
            opt.eeg_path = opt.root_path / opt.eeg_path
...
def get_train_utils(opt, model_parameters, eeg_parameters):
    ...
    if eeg_parameters is None:
        train_data = get_training_data(opt.video_path, opt.annotation_path,
                                       opt.dataset, opt.input_type, opt.file_type,
                                       spatial_transform, temporal_transform)
    else:

        train_data = get_training_eeg_data(opt.video_path, opt.eeg_path, opt.annotation_path,
                                          opt.dataset, opt.input_type, opt.file_type,
                                          spatial_transform, temporal_transform)
    ...
    if opt.use_eeg:
        optimizer_eeg = SGD(eeg_parameters,
                           lr=opt.learning_rate,
                           momentum=opt.momentum,
                           dampening=dampening,
                           weight_decay=opt.weight_decay,
    ...
    return (train_loader, train_sampler, train_logger, train_batch_logger, optimizer, optimizer_eeg, scheduler)
...
def main_worker(index, opt):
    ...
    #####################################################################################
    ### here add a classifier to predict videos and eegs
    if opt.inference is False:
        ### define loss
        criterion = CrossEntropyLoss().to(opt.device)

        if opt.use_eeg or opt.use_image:
            criterion_jsd = JSDLoss(weight=0.5)
            criterion_khn_eeg=KhNloss(margin=0.5)
            criterion_sta=STALOSS()
        ...
        if opt.use_eeg:
            ...
            feature_dim = 512
            if opt.pretrain_path is not None:
                joint_prediction_eeg = generate_prediction(feature_dim, opt.n_finetune_classes, normalization=True)
            else:
                joint_prediction_eeg = generate_prediction(feature_dim, opt.n_classes, normalization=True)
            if opt.resume_path is not None:
                aux_checkpoint = Path(os.path.join(str(opt.resume_path.parent), str(opt.resume_path.name[:-4] + 'eeg.pth')))
                joint_prediction_eeg = resume_model(aux_checkpoint, opt.arch, joint_prediction_eeg)

            joint_prediction_eeg = make_data_parallel(joint_prediction_eeg, opt.distributed, opt.device)
            eeg_para = joint_prediction_eeg.parameters()
            joint_prediction_eeg.cuda()
        ...
        #################################################################################
        (train_loader, train_sampler, train_logger, train_batch_logger, optimizer, optimizer_eeg, scheduler) = \
            get_train_utils(opt, model_parameters=parameters, av_parameters=eeg_para)
    ...
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        if not opt.no_train:
            ...
            if optimizer_eeg is not None :
                train_eeg_epoch(epoch=i, data_loader=train_loader, model=model, joint_prediction_eeg=joint_prediction_eeg,
                              criterion=criterion, criterion_jsd=criterion_jsd, criterion_ct_eeg=criterion_khn_eeg,certerion_sta=criterion_sta
                              optimizer=optimizer, optimizer_eeg=optimizer_eeg,
            ...
            if i % opt.checkpoint == 0 and opt.is_master_node:
                ...
                if opt.use_eeg:
                    save_file_path = opt.result_path / 'save_{}_eeg.pth'.format(i)
                    save_checkpoint(save_file_path, i, opt.arch, joint_prediction_eegd, optimizer, scheduler)
