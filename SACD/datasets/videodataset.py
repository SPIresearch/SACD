...
def get_av_database(data, subset, root_path, eeg_path, video_path_formatter, eeg_path_formatter):
    video_ids = []
    video_paths = []
    eeg_paths = []
    annotations = []
    ...
    ## filter the classes
    av_classes = sorted(eeg_path.iterdir())
    ...
    for key, value in data['database'].items():
        ...
        if value['annotations']['label'] in av_classes:
            ...
            if this_subset == subset:
                ...
                eeg_file = eeg_path_formatter(eeg_path, label, key)
                if os.path.isfile(eeg_file):
                    eeg_paths.append(eeg_path_formatter(eeg_path, label, key))
                else:
                    eeg_paths.append(None)
...
class EEGVideoDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.data, self.class_names, self.n_videos = self.__make_av_dataset(
            root_path, eeg_path, annotation_path, subset, video_path_formatter, eeg_path_formatter)
        ...
        if video_loader is None:
            ...
            self.loader = video_loader
        self.eeg_loader = eegFeatureLoader()
        self.target_type = target_type

    def __make_av_dataset(self, root_path, eeg_path, annotation_path, subset,
                       video_path_formatter, eeg_path_formatter):
        with annotation_path.open('r') as f:
            data = json.load(f)
        video_ids, video_paths, eeg_paths, annotations = get_av_database(
            data, subset, root_path, eeg_path, video_path_formatter, eeg_path_formatter)
    ...
    def __loading(self, path, frame_indices, eeg_filename):
        ...
        if eeg is None:
            eeg_dim = 512
            eeg = np.zeros(eeg_dim, dtype=np.float32)
